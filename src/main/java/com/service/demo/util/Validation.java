	...
	public void notesValidation(NotesDto notesDto) {
		...
		if (ObjectUtils.isEmpty(notesDto)) {
			...
			if (ObjectUtils.isEmpty(notesDto.getTitle())) {
				// throw new IllegalArgumentException("name field is null or Empty");
				map.put("title :", "title field shuold 't be null or Empty");
			} else {
				if (notesDto.getTitle().length() < 10) {
					map.put("title :", "name length min 10");
				}
				if (notesDto.getTitle().length() > 100) {
					map.put("title :", "name length max 100");
				}
			}
			if (ObjectUtils.isEmpty(notesDto.getDescription())) {
				...
				if (notesDto.getDescription().length() > 100) {
					map.put("Description :", "Description length max 100");
				}
			}
